...
static bool invert_homography(const double H[9], double inv[9]) {
...
static bool apply_homography(const double H[9], double x, double y, double *xo, double *yo) {
...
static Options parse_options(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (strcmp(arg, "-h") == 0 || strcmp(arg, "--help") == 0) {
      ...
      for (int k = 0; k < 9; ++k) {
        opt.H[k] = parse_double_arg(argv[++i], "--homography");
      }
  ...
  if (!invert_homography(opt.H, opt.Hinv)) {
...
static bool in_bounds(double x, double y, long width, long height) {
...
static bool sample_bilinear(const Image *img, double x, double y, float *out) {
  if (x < 0.0 || y < 0.0 || x > (double)(img->width - 1) || y > (double)(img->height - 1)) return false;
...
static bool fit_node(const Image *ref, const Image *target, const unsigned char *ref_mask,
                     const unsigned char *target_mask, long cx, long cy, const Options *opt,
                     float *scale_out, float *offset_out, float *ref_bg_out,
                     float *target_bg_out, int *samples_out) {
  int radius = opt->window_size / 2;
  long x0 = cx - radius;
  long x1 = cx + radius;
  long y0 = cy - radius;
  long y1 = cy + radius;
  ...
  for (long y = y0; y <= y1; ++y) {
    ...
    for (long x = x0; x <= x1; ++x) {
      ...
      if (ref_mask && ref_mask[idx]) continue;
      double tx = 0.0, ty = 0.0;
      if (!apply_homography(opt->Hinv, (double)x, (double)y, &tx, &ty)) continue;
      if (!in_bounds(tx, ty, target->width, target->height)) continue;
      if (mask_at(target_mask, target->width, target->height, tx, ty)) continue;
      float t = 0.0f;
...
static int estimate_grid(const Image *ref, const Image *target, const unsigned char *ref_mask,
                         const unsigned char *target_mask, const Options *opt, Grid *grid) {
  int valid_count = 0;
  ...
  for (int gy = 0; gy < grid->ny; ++gy) {
    for (int gx = 0; gx < grid->nx; ++gx) {
      ...
      int samples = 0;
      if (fit_node(ref, target, ref_mask, target_mask, cx, cy, opt, &grid->scale[gi],
...
static bool bilinear_grid_value(const float *values, const Grid *grid, const Options *opt,
                                double x, double y, long width, long height, float *out) {
  if (!in_bounds(x, y, width, height)) return false;
  double gx_f = x / (double)opt->grid_spacing;
  double gy_f = y / (double)opt->grid_spacing;
...
static bool extrapolated_grid_value(const float *values, const Grid *grid, const Options *opt,
                                    double x, double y, long width, long height, float *out) {
  if (bilinear_grid_value(values, grid, opt, x, y, width, height, out)) return true;

  double gx_f = x / (double)opt->grid_spacing;
  double gy_f = y / (double)opt->grid_spacing;
  int cx = clamp_int((int)llround(gx_f), 0, grid->nx - 1);
  int cy = clamp_int((int)llround(gy_f), 0, grid->ny - 1);
...
static void apply_correction(const Image *target, const Image *ref, const Grid *grid,
                             const Options *opt, float *corrected, float *scale_map,
                             float *offset_map, float *ref_bg_map, float *target_bg_map) {
  ...
  for (long y = 0; y < height; ++y) {
    for (long x = 0; x < width; ++x) {
      ...
      float offset = 0.0f;
      float ref_bg = 0.0f;
      float target_bg = 0.0f;
      if (apply_homography(opt->H, (double)x, (double)y, &rx, &ry) &&
          extrapolated_grid_value(grid->scale, grid, opt, rx, ry, ref->width, ref->height, &scale) &&
          extrapolated_grid_value(grid->offset, grid, opt, rx, ry, ref->width, ref->height, &offset)) {
        extrapolated_grid_value(grid->ref_bg, grid, opt, rx, ry, ref->width, ref->height, &ref_bg);
        extrapolated_grid_value(grid->target_bg, grid, opt, rx, ry, ref->width, ref->height, &target_bg);
      }
      scale_map[idx] = scale;
      offset_map[idx] = offset;
      if (ref_bg_map) ref_bg_map[idx] = ref_bg;
      if (target_bg_map) target_bg_map[idx] = target_bg;
...
static void write_report(const char *path, const Options *opt, const Image *ref,
                         const Image *target, const Grid *grid, int initial_valid,
                         long ref_masked, long target_masked, double elapsed,
                         const float *scale_map, const float *offset_map) {
  FILE *f = fopen(path, "w");
  if (!f) {
  ...
  min_max_float(scale_map, (size_t)target->width * (size_t)target->height, &scale_min, &scale_max);
  min_max_float(offset_map, (size_t)target->width * (size_t)target->height, &offset_min, &offset_max);
  ...
  fprintf(f,
          ...
          "  \"openmp_threads\": %d\n"
          "}\n",
          ref->width, ref->height, target->width, target->height, opt->grid_spacing,
          opt->window_size, opt->min_samples, grid->nx, grid->ny, initial_valid,
...
int main(int argc, char **argv) {
  ...
  unsigned char *ref_mask = read_mask_fits(opt.ref_mask_path, ref.width, ref.height);
  unsigned char *target_mask = read_mask_fits(opt.target_mask_path, target.width, target.height);
  long ref_masked = count_masked(ref_mask, ref_npixels);
  long target_masked = count_masked(target_mask, target_npixels);

  Grid grid = create_grid(ref.width, ref.height, opt.grid_spacing);
  int initial_valid = estimate_grid(&ref, &target, ref_mask, target_mask, &opt, &grid);
  int total_nodes = grid.nx * grid.ny;
  ...
  float *corrected = (float *)checked_malloc((size_t)target_npixels * sizeof(float));
  float *scale_map = (float *)checked_malloc((size_t)target_npixels * sizeof(float));
  float *offset_map = (float *)checked_malloc((size_t)target_npixels * sizeof(float));
  float *ref_bg_map = opt.save_backgrounds ? (float *)checked_malloc((size_t)target_npixels * sizeof(float)) : NULL;
  float *target_bg_map = opt.save_backgrounds ? (float *)checked_malloc((size_t)target_npixels * sizeof(float)) : NULL;
  apply_correction(&target, &ref, &grid, &opt, corrected, scale_map, offset_map, ref_bg_map, target_bg_map);
  write_fits_float(opt.out_path, target.width, target.height, corrected);

  if (opt.diag_dir) {
    char *scale_path = join_path(opt.diag_dir, "scale_map.fits");
    char *offset_path = join_path(opt.diag_dir, "offset_map.fits");
    write_fits_float(scale_path, target.width, target.height, scale_map);
    write_fits_float(offset_path, target.width, target.height, offset_map);
    free(scale_path);
    free(offset_path);
    if (opt.save_backgrounds) {
      ...
      write_fits_float(ref_bg_path, target.width, target.height, ref_bg_map);
      write_fits_float(target_bg_path, target.width, target.height, target_bg_map);
      free(ref_bg_path);
      free(target_bg_path);
    }
  }
  if (opt.report_path) {
    write_report(opt.report_path, &opt, &ref, &target, &grid, initial_valid,
                 ref_masked, target_masked, now_seconds() - start, scale_map, offset_map);
  }
