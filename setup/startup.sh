...
set_time_location() {
  ...
  indi_setprop "Star Adventurer GTi.GEOGRAPHIC_COORD.LAT=$SITE_LATITUDE;LONG=$SITE_LONGITUDE;ELEV=$SITE_ELEVATION"
  ...
  # indi_setprop "Star Adventurer GTi.TIME_UTC.UTC=$CURRENT_UTC_TIME"
}    

INDI_RUNNING=$(pgrep indiserver)
if [ -z "$INDI_RUNNING" ]; then
    echo "Starting INDI server"
    screen -mdS indi indiserver indi_staradventurergti_telescope indi_asi_focuser
    sleep 1
    echo "Connecting to mount"
    indi_setprop "Star Adventurer GTi.CONNECTION.CONNECT=On"    
    retcode=$?
    ...
    CONNECTED=$(indi_getprop "Star Adventurer GTi.CONNECTION.CONNECT" | grep -o "CONNECT=On")
    if [ -z "$CONNECTED" ]; then
        echo "Connecting to mount"
        indi_setprop "Star Adventurer GTi.CONNECTION.CONNECT=On"
        retcode=$?
